...
async loadProductImages(productId) {
  try {
    const response = await fetch(`/api/admin/productos/${productId}/imagenes`);
...
renderImageGallery() {
    ...
    html += `
      <div class="col-6 col-md-4" data-image-id="${img.id}">
        <div class="card position-relative">
          ${img.es_principal ? '<span class="badge bg-success position-absolute top-0 end-0 m-1">Principal</span>' : ''}
          <img src="${img.imagen_url}" class="card-img-top" style="height: 150px; object-fit: cover;">
          <div class="card-body p-2">
            <div class="btn-group btn-group-sm w-100">
              ${!img.es_principal ? `<button class="btn btn-outline-primary" onclick="adminPanel.setMainImage(${img.id})">
                <i class="fas fa-star"></i>
              </button>` : '<button class="btn btn-primary" disabled><i class="fas fa-star"></i></button>'}
              <button class="btn btn-outline-danger" onclick="adminPanel.deleteImage(${img.id})">
                <i class="fas fa-trash"></i>
              </button>
            </div>
          </div>
  ...
  document.getElementById('addMoreImages')?.addEventListener('change', (e) => {
    this.handleMultipleImageUpload(e);
  });
}
...
handleMultipleImageUpload(event) {
  ...
  const totalImages = this.productImages.length + this.filesToUpload.length + files.length;

  if (totalImages > 5) {
    this.showAlert(`Máximo 5 imágenes. Actualmente tienes ${this.productImages.length + this.filesToUpload.length}`, "warning");
    return;
  }
...
async setMainImage(imageId) {
  ...
  try {
    const response = await fetch(`/api/admin/productos/${this.editingProduct}/imagenes/${imageId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
getOrCreateGalleryContainer() {
  ...
  if (!container) {
    ...
    label.innerHTML = '<i class="fas fa-images me-2"></i>Galería de Imágenes del Producto';
    
    container.prepend(label);
    
    // Insertar después del campo de TikTok
    const tiktokField = document.getElementById('productoTiktokUrl').closest('.mb-3');
...
showEmptyGallery() {
  ...
  document.getElementById('firstImageUpload')?.addEventListener('change', (e) => {
    this.handleMultipleImageUpload(e);
  });
}
