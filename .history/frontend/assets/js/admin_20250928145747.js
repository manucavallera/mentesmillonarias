...
class AdminPanel {
  ...
  async saveProduct() {
    ...
    // Obtener imagen existente (para edición)
    ...
    if (imagenFile) {
      ...
      if (nuevaImagen) {
        imagen_url = nuevaImagen; // Usar la nueva imagen
      } else {
        return; // Error en la subida
      }
    }
    // Si no hay archivo nuevo pero estamos editando, mantener imagen existente
    // (imagen_url ya tiene el valor del campo URL)
    ...
    const productData = {
      ...
      imagen_url: imagen_url || null, // Usar imagen existente o nueva
    };
  ...
  async editProduct(id) {
    try {
      ...
      // CRÍTICO: Preservar imagen actual en el campo URL
      const imagenUrlField = document.getElementById("productoImagen");
      imagenUrlField.value = producto.imagen_url || "";

      console.log(
        "✅ Campo productoImagen rellenado con:",
        imagenUrlField.value
      );
        ...
        if (preview && previewContainer) {
          ...
          <div class="text-center mb-3">
            ...
            <div class="small text-success mt-1">
              URL preservada: ${producto.imagen_url.substring(0, 50)}...
            </div>
          </div>
        `;
