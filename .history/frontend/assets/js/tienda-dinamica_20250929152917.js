...
class TiendaDinamica {
  ...
  renderProductos() {
    ...
    <small class="text-${producto.stock > 0 ? "success" : "danger"}">
  ...
  this.carrito.push({
    ...
    precio_rebajado: producto.precio_rebajado ? parseFloat(producto.precio_rebajado) : null,
    cantidad: 1,
  ...
  async iniciarCompraWhatsApp() {
    ...
    // NUEVO: Guardar pedido en la base de datos
    try {
  ...
  async crearPedidoEnBD() {
    ...
    const pedidoData = {
      ...
      total: total,
      estado: "pendiente",
      cliente_nombre: "Pendiente por WhatsApp",
      cliente_email: "",
